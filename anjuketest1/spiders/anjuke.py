...
class AnjukeSpider(scrapy.Spider):
    ...
    all_urls = "https://www.anjuke.com/sy-city.html"
    cookies = {
    ...
    def start_requests(self):
        # 入口
        yield scrapy.Request(url=self.all_urls,cookies=self.cookies,callback=self.first_parse)
    def first_parse(self, response):
        a = re.findall(r'class="label_letter">A</label>(.*?)class="label_letter">其他</label>', response.text, re.S)
        city_urls = re.findall(r'href="(.*?)">', a[0])
        # 获取全部城市url
        for city_url in city_urls:
            city_url_new = city_url+'/sale/o5/'
            # yield scrapy.Request(url=city_url_new,cookies=self.cookies,callback=self.second_parse,priority=1)
            yield scrapy.Request(url=city_url_new,cookies=self.cookies,callback=self.thrid_parse,priority=2)
    # def second_parse(self,response):
    #     # 获取区域url
    #     quyu_urls = response.xpath('//*[@id="content"]/div[@class="div-border items-list"]/div[@class="items"]/span[@class="elems-l"]/a/@href').extract()
    #     for quyu_url in quyu_urls:
    #         yield scrapy.Request(url=quyu_url,cookies=self.cookies,callback=self.thrid_parse,priority=2)
